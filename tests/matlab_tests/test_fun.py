...
from ndi.fun.doc import all_types, find_fuid, diff as doc_diff
...
def _make_demo_doc(name: str = 'test', value: int = 42,
                   session_id: str = '') -> Document:
...
def _make_session_with_docs(tmp_path, ref, doc_specs):
    ...
    for name, value in doc_specs:
        ...
        results = session.database_search(
            Query('base.id') == doc.document_properties['base']['id']
        )
...
class TestFindFuid:
    ...
    def test_find_known_fuid(self, tmp_path):
        ...
        assert found_name == 'filename1.ext', \
            f'File name should be filename1.ext, got {found_name}'
    ...
    def test_find_fuid_in_populated_session(self, tmp_path):
        ...
        for i in range(1, 4):
            ...
            if i == 2:
                ...
                target_fuid = doc.document_properties['files']['file_info'][0]['locations'][0]['uid']
...
class TestVStack:
    ...
    def test_empty_df_with_columns_stacked_with_data(self):
        ...
        """
        df_empty = pd.DataFrame({'a': pd.Series(dtype='float64'),
