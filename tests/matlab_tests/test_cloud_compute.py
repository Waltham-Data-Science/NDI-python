...
class TestCompute:
    ...
    def test_advanceSession_uses_advance_route(self):
        """advanceSession hits POST /compute/{sessionId}/advance, not /finalize."""
        from ndi.cloud.api.compute import advanceSession

        client = MagicMock()
        client.post.return_value = {"status": "ok"}

        advanceSession("session-abc-123", client=client)
