...
class TestDeleteSession:
    ...
    def test_delete_no_confirm(self, tmp_path):
        ...
        result = session.delete_session_data_structures(
            are_you_sure=False, ask_user=False
        )

        assert ndi_dir.exists(), \
            '.ndi directory should still exist after unconfirmed delete'
    ...
    def test_delete_confirm(self, tmp_path):
        ...
        result = session.delete_session_data_structures(
            are_you_sure=True, ask_user=False
        )

        assert not ndi_dir.exists(), \
            '.ndi directory should be gone after confirmed delete'
    ...
    def test_delete_preserves_data_files(self, tmp_path):
        ...
        session.delete_session_data_structures(
            are_you_sure=True, ask_user=False
        )
...
class TestIsIngestedInDataset:
    ...
    def test_standalone_session_not_in_dataset(self, tmp_path):
        ...
        assert session.id() not in session_ids, \
            'Standalone session should not be in dataset'
    ...
    def test_ingested_session_in_dataset(self, tmp_path):
        ...
        assert session.id() in session_ids, \
            'Ingested session should appear in dataset session list'
...
class TestSessionBasics:
    ...
    def test_newdocument(self, tmp_path):
        ...
        doc = session.newdocument('demoNDI', **{
            'base.name': 'test',
            'demoNDI.value': 99,
        })

        assert doc.session_id == session.id(), \
            'newdocument should set session_id'
