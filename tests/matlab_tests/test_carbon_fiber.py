...
def carbon_fiber_dataset(tmp_path_factory):
    """Load 743 JSON docs into a ndi_dataset object (once per session)."""
...
class TestDatasetLoading:
    ...
    def test_load_document_count(self, carbon_fiber_dataset):
        """At least 743 documents load (ndi_dataset init adds 1 session doc)."""
        from ndi.query import ndi_query

        docs = carbon_fiber_dataset.database_search(ndi_query("").isa("base"))
    ...
    def test_document_type_counts(self, carbon_fiber_dataset):
        ...
        for dtype, expected in EXPECTED_TYPE_COUNTS.items():
            ...
            if dtype == "session":
                # ndi_dataset init may add 1 extra session doc
                assert (
    ...
    def test_getDocTypes(self, carbon_fiber_dataset):
        ...
        # 27 from JSON + session doc auto-created by ndi_dataset init = 28
...
class TestElements:
    ...
    def test_element_count(self, carbon_fiber_dataset):
        ...
        docs = carbon_fiber_dataset.database_search(ndi_query("").isa("element"))
        assert len(docs) == 20
    ...
    def test_element_depends_on_subject(self, carbon_fiber_dataset):
        ...
        elements = carbon_fiber_dataset.database_search(ndi_query("").isa("element"))
        subjects = carbon_fiber_dataset.database_search(ndi_query("").isa("subject"))
...
class TestNeurons:
    ...
    def test_neuron_count(self, carbon_fiber_dataset):
        ...
        docs = carbon_fiber_dataset.database_search(ndi_query("").isa("neuron_extracellular"))
        assert len(docs) == 17
...
class TestStimulusResponses:
    ...
    def test_stimulus_response_count(self, carbon_fiber_dataset):
        ...
        docs = carbon_fiber_dataset.database_search(ndi_query("").isa("stimulus_response_scalar"))
        assert len(docs) == 138
...
class TestTuningCurves:
    ...
    def test_tuningcurve_count(self, carbon_fiber_dataset):
        ...
        docs = carbon_fiber_dataset.database_search(ndi_query("").isa("tuningcurve_calc"))
        assert len(docs) == 126
    ...
    def test_oridirtuning_count(self, carbon_fiber_dataset):
        ...
        docs = carbon_fiber_dataset.database_search(ndi_query("").isa("oridirtuning_calc"))
        assert len(docs) == 34
...
class TestEpochStructure:
    ...
    def test_element_epoch_count(self, carbon_fiber_dataset):
        ...
        docs = carbon_fiber_dataset.database_search(ndi_query("").isa("element_epoch"))
        assert len(docs) == 46
    ...
    def test_epochfiles_ingested_count(self, carbon_fiber_dataset):
        ...
        docs = carbon_fiber_dataset.database_search(ndi_query("").isa("epochfiles_ingested"))
        assert len(docs) == 10
    ...
    def test_stimulus_presentation_count(self, carbon_fiber_dataset):
        ...
        docs = carbon_fiber_dataset.database_search(ndi_query("").isa("stimulus_presentation"))
        assert len(docs) == 5
...
class TestOpenMinds:
    ...
    def test_openminds_count(self, carbon_fiber_dataset):
        ...
        docs = carbon_fiber_dataset.database_search(ndi_query("").isa("openminds"))
    ...
    def test_openminds_stimulus_count(self, carbon_fiber_dataset):
        ...
        docs = carbon_fiber_dataset.database_search(ndi_query("").isa("openminds_stimulus"))
        assert len(docs) == 10
...
class TestCrossDocumentRelationships:
    ...
    def test_depends_on_references_exist(self, carbon_fiber_dataset):
        ...
        all_docs = carbon_fiber_dataset.database_search(ndi_query("").isa("base"))
    ...
    def test_session_id_consistency(self, carbon_fiber_dataset):
        ...
        for doc in all_docs:
            ...
            if sid:
                session_ids.add(sid)
        # 2 original + 1 from ndi_dataset init
    ...
    def test_neuron_element_chain(self, carbon_fiber_dataset):
        ...
        neurons = carbon_fiber_dataset.database_search(ndi_query("").isa("neuron_extracellular"))
        elements = carbon_fiber_dataset.database_search(ndi_query("").isa("element"))
...
class TestDAQInfrastructure:
    ...
    def test_daqsystem_count(self, carbon_fiber_dataset):
        ...
        docs = carbon_fiber_dataset.database_search(ndi_query("").isa("daqsystem"))
        assert len(docs) == 3
    ...
    def test_daqreader_count(self, carbon_fiber_dataset):
        ...
        docs = carbon_fiber_dataset.database_search(ndi_query("").isa("daqreader"))
        assert len(docs) == 3
    ...
    def test_filenavigator_count(self, carbon_fiber_dataset):
        ...
        docs = carbon_fiber_dataset.database_search(ndi_query("").isa("filenavigator"))
        assert len(docs) == 3
    ...
    def test_syncgraph_exists(self, carbon_fiber_dataset):
        ...
        docs = carbon_fiber_dataset.database_search(ndi_query("").isa("syncgraph"))
        assert len(docs) == 1
    ...
    def test_syncrule_count(self, carbon_fiber_dataset):
        ...
        docs = carbon_fiber_dataset.database_search(ndi_query("").isa("syncrule"))
        assert len(docs) == 2
