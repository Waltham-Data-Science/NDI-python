"""Generate symmetry artifacts for a blank marderlab NDI session.

Python equivalent of:
    tests/+ndi/+symmetry/+makeArtifacts/+session/blankSessionMarderlab.m

This test creates a blank NDI session configured with the marderlab DAQ
systems (from ``ndi_common/daq_systems/marderlab/``), then persists the
session database, a ``sessionSummary.json`` manifest, and individual JSON
representations of every document into:

    <tempdir>/NDI/symmetryTest/pythonArtifacts/session/blankSessionMarderlab/
             testBlankSessionMarderlab/
...
ARTIFACT_DIR = (
    PYTHON_ARTIFACTS
    / "session"
    / "blankSessionMarderlab"
    / "testBlankSessionMarderlab"
)


class TestBlankSessionMarderlab:
    """Mirror of ndi.symmetry.makeArtifacts.session.blankSessionMarderlab."""

    @pytest.fixture(autouse=True)
    def _setup(self, tmp_path):
        """Create a blank session with marderlab DAQ systems."""
    ...
    def test_blank_session_marderlab(self):
        """Export the blank marderlab session to the shared symmetry artifact directory."""
