...
class TestTheCloudPalette:
    """napari ships a dark theme and its docks inherit it, so the panels
    came out grey against every other NDI applet -- white bodies under a
    navy header. The palette is not invented in the stylesheet; it comes
    from ndi.gui.cloud_colors, the same triplets ndi.gui.cloudColors holds
    on the MATLAB side, so the two stay one look rather than two
    approximations of one."""

    def test_the_stylesheet_carries_the_cloud_colours(self):
        from ndi.gui.app.genepyramid.controls import cloudPaperStyle
        ...
        assert hexOf(c.off_white) in css, "panel bodies are the cloud off-white"
        assert hexOf(c.dark_blue) in css, "text is the cloud navy"
        assert hexOf(c.white) in css, "fields are white"

    def test_a_widget_that_refuses_a_stylesheet_costs_the_colour_only(self):
        """Styling is never worth the panel -- the same trade
        ndi.gui.app.GEFManager.paper makes on the MATLAB side."""
        from ndi.gui.app.genepyramid.controls import applyCloudStyle

        class Refuses:
            def setStyleSheet(self, _css):
                raise RuntimeError("no stylesheets here")

        assert applyCloudStyle(Refuses()) is False

    def test_a_widget_that_takes_it_reports_success(self):
        from ndi.gui.app.genepyramid.controls import applyCloudStyle
        ...
        w = Takes()
        assert applyCloudStyle(w) is True
        assert w.css
