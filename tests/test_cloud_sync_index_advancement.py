...
def _downloads_that_fail(monkeypatch, succeed):
    ...
    def fake_download(cloud_id, ndi_to_api, ids_to_download, *, client=None):
        ...
        # Whole documents, as the cloud returns them: a downloaded document
        # is ingested into the dataset, so an id-only stub would be filed
        # under no id and never become local.
        return [{**make_document(i), "ndiId": i} for i in got], missing

    monkeypatch.setattr(ops, "downloadNdiDocuments", fake_download)


def _seed(tmp_path, local, remote):
    """A dataset holding *local* documents, and an index recording the last sync.

    The two are separate now. ``local`` is what the dataset actually holds --
    the sync functions enumerate it -- while the index only remembers what
    the previous run saw, which is what makes "added since last sync"
    answerable at all (NDI-python#232).
    """
    idx = SyncIndex()
    idx.update(list(local), list(remote))
    idx.write(tmp_path)
    return FakeDataset(tmp_path, [make_document(i) for i in local])
...
class TestDownloadNew:
    ...
    def test_a_successful_run_records_everything(self, tmp_path, monkeypatch):
        ...
        ops.downloadNew(ds, "cloud-1", SyncOptions(verbose=False))
        idx = SyncIndex.read(tmp_path)
    ...
    def test_remote_documents_already_local_stay_recorded(self, tmp_path, monkeypatch):
        ...
        ds = _seed(tmp_path, ["have-it"], ["have-it"])
...
class TestMirrorFromRemote:
    def test_a_failed_download_is_retried_on_the_next_run(self, tmp_path, monkeypatch):
        ...
        ops.mirrorFromRemote(ds, "cloud-1", SyncOptions(verbose=False))
        idx = SyncIndex.read(tmp_path)
        assert idx.local_doc_ids_last_sync == ["ok-1"]
    ...
    def test_local_only_documents_are_still_deleted_and_dropped(self, tmp_path, monkeypatch):
        ds = _seed(tmp_path, ["gone", "stays"], ["gone", "stays"])
...
class TestMirrorToRemote:
    def _uploads_that_fail(self, monkeypatch, succeed):
        ...
        class FakeDocsApi:
            ...
            def addDocument(cloud_id, doc, *, client=None):
                # A whole document arrives now, not {"ndiId": id}.
                assert "base" in doc, "the upload must send the document, not just its id"
                if doc["base"]["id"] not in succeed:
    ...
    def test_a_failed_remote_deletion_stays_on_the_remote_side(self, tmp_path, monkeypatch):
        ...
        ds = _seed(tmp_path, ["mine"], ["mine", "undeletable"])
