...
class DAQSystem(Ido):
    ...
    def __init__(
        self,
        name: str = '',
        filenavigator: Optional[Any] = None,
        daqreader: Optional[DAQReader] = None,
        daqmetadatareaders: Optional[List[Any]] = None,
    ...
    def _load_from_document(self, session: Any, document: Any) -> None:
        ...
        metadata_ids = document.dependency_value_n(
        ...
        for mid in metadata_ids:
            ...
            if len(m_docs) == 1:
                ...
                metadata_readers.append(
                    MetadataReader(session=session, document=m_docs[0])
                )
    ...
    @property
    def daqreader(self) -> Optional[DAQReader]:
        """Get the DAQ reader."""
    ...
    def set_daqmetadatareaders(
        ...
        for i, r in enumerate(readers):
            if not isinstance(r, MetadataReader):
                raise TypeError(
                    f"Element {i} is not a MetadataReader instance"
                )
    ...
    def getepochprobemap(
        self,
        epoch: int,
        filenavepochprobemap: Optional[Any] = None,
    ) -> Any:
        """
    ...
    def getmetadata(
        ...
        if channel < 1 or channel > max(n_readers, 1):
            raise ValueError(
                f"Metadata channel out of range: 1..{max(n_readers, 1)}"
            )
    ...
    def ingest(self) -> Tuple[bool, List[Any]]:
        ...
        for entry in et:
            ...
            if self._daqreader is not None:
                reader_docs = self._daqreader.ingest_epochfiles(
                    epochfiles,
                    entry['epoch_id']
                )
