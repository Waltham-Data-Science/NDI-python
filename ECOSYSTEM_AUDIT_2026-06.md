...
| C8b | **`ndi.neuron` is unconstructable → every sorted unit silently unreadable.** MATLAB `element.m:524` writes `class(obj)` = `'ndi.neuron'` into `element.ndi_element_class`; `_document_to_object` does `get_class('ndi.neuron')` → `None` → raises `Unknown element class: 'ndi.neuron'`, which `getelements` swallows (`try/except: pass`), so `getelements()` returns **zero neurons** with no error. Two root causes: (1) `ndi_neuron` never overrides `ndi_element_class()`, so it inherits `"ndi.element"` — meaning Python-written neurons are *also* mislabeled and round-trip as plain elements; (2) `ndi_neuron` is absent from `_build_registry`. Prereq for C8's neuron read path. **Same pattern must be checked for `ndi_element_timeseries`.** | `class_registry.py:66-73` · `neuron.py` (no `ndi_element_class` override) · `session_base.py:1129-1134,952-957` vs MATLAB `element.m:524` | (1) add `def ndi_element_class(self): return "ndi.neuron"` to `neuron.py`; (2) add `ndi_neuron` to the registry elements tuple; (3) make `getelements` surface (log/raise) construction failures instead of `except: pass`. Add `neuron` to the bridge (§5). Lands in PR8; prerequisite to PR4. |
...
- **Hard-fail principle:** 53 silent `try/except: pass` sites (e.g. `client.py:289`, `database_fun.py:73-79`) — at minimum `logger.debug` the exception; convert genuine errors to raises. **`session_base.py:952-957` specifically must surface, not just log** — it silently drops every object that fails `_document_to_object` (the mechanism that hid C8b's neuron failure); a doc that `isa('element')` but won't construct is a registry/parity gap that should be loud.
...
syncgraph branches + underlying epochs + rule daqsystem (C5–C7) · **`ndi.neuron` registration + `ndi_element_class` override + un-swallow getelements (C8b — prereq for the neuron read)** · `readtimeseries`/VHSB (C8) · `epochprobemap_daqsystem` format (C10) · DID `isa` (§6.1-1) · `dependency_value_n` · `system_mfdaq` analog-event types · stimulator `pairOnOff` · `syncTriggerTrains` + `randomPulses` algorithms · ontology providers (UBERON/NCIT + registry sync) · cloud contract drift items (§3.4-15/16) + 5xx retry.
