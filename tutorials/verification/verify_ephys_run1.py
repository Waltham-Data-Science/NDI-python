...
def main():
    ...
    subj = next((x.get("value") for x in (pv_doc.document_properties.get("depends_on") or [])
                 if x.get("name") == "subject_id"), None)
    ...
    print(f"  Vm range [{np.nanmin(dataVm):.3g},{np.nanmax(dataVm):.3g}] (mV?)  "
    ...
    ok = (numSteps >= 3 and numTimepoints > 100
          and np.nanmin(dataVm) > -200 and np.nanmax(dataVm) < 100)
